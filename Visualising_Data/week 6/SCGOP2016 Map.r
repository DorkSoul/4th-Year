...
usshapefile <- file.path(mydata,'usshapefile')
...
# Read in shapefile of Irish counties
ireland_counties <- st_read("ireland_counties.shp")

# Get the bounding box of Ireland
ireland_bbox <- st_bbox(ireland_counties)

# Get the map of Ireland using Google Maps
ireland_map <- get_map(location = ireland_bbox, zoom = 7, maptype = "terrain")

# Plot the map of Ireland with county boundaries
ggmap(ireland_map) +
  geom_sf(data = ireland_counties, color = "white", fill = "lightgray") +
  coord_map()

# Add a point to highlight Dublin
dublin_location <- geocode("Dublin, Ireland")
ggmap(ireland_map) +
  geom_sf(data = ireland_counties, color = "white", fill = "lightgray") +
  geom_point(aes(x = lon, y = lat), data = dublin_location, color = "red", size = 4) +
  coord_map()